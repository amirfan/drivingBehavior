...
//    spark.udf.register( "theroyDist" , theroyDist )
//    spark.udf.register( "theroyDist2" , theroyDist2 )
...
//      result.write.mode(SaveMode.Overwrite).option("user",LoadConfig.getHdfsUser()).option("header", "true").csv(fsPath+"rapid/"+mon+"/")
    }
  }
  ...
  def calRapidSpeed(spark: SparkSession,dataSet: DataFrame): DataFrame = {
    ...
    val sql0 =
      s"""
         ...
         |from tmp_table
         |where lspeed<=80
         |having (now - pre)>=5 and (now - pre)<= 20
       """.stripMargin
    ...
    val sql1 =
      s"""
         ...
         |roadcross,
         |lead(roadcross,1,0) over(partition by jsy order by longtime) as latcross,
         |roadsection,
         |flag,
         |direcflag,
         ...
         |lead(orientation,1,0) over(partition by jsy order by longtime) as latorien
         |from tab_tmp0
       """.stripMargin
    ...
    spark.sql(sq).repartition(1).write.mode(SaveMode.Append).option("user",LoadConfig.getHdfsUser()).option("header", "true").csv("hdfs://192.168.100.177:8020/user/root/VDP_bus/2017analyse/451/rapidStat/")
    ...
    val sql7 =
      s"""
         |select
         |t.jsy,t.llat,t.llon,t.wayname,t.waytype,t.precross,t.roadcross,t.latcross,t.roadsection,flag,direcflag,
         |t.pretime,t.prespeed,t.preorien,t.longtime,t.lspeed,t.orientation,
         |t.lattime,t.latspeed,t.latorien,t.rapidMark
         |from tab_tmp6 as t
         |where t.rapidMark <> 0 and t.roadsection is not null
       """.stripMargin
    ...
    val sql9 =
      s"""
         ...
         |from tab_tmp8
         |where orient>45  -- and avgSpeed>6.98/5.55
       """.stripMargin
    ...
    val sql11 =
      s"""
       ...
       """.stripMargin
    spark.sql(sql11).createOrReplaceTempView("tab_tmp11")
    ...
    spark.sql(sq000).repartition(1).write.mode(SaveMode.Append).option("user",LoadConfig.getHdfsUser()).option("header", "true").csv("hdfs://192.168.100.177:8020/user/root/VDP_bus/2017analyse/451/rapidAngle/")
    ...
    val sql13 =
      s"""
         ...
         |t2.angleMark
         |from
         |(select jsy,longtime,roadsection,rapidMark from tmp_rapid) as t1
         |full outer join (select jsy,longtime,iscross,angleMark from tmp_angle) as t2

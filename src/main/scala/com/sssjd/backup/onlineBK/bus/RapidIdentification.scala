...
object RapidIdentification {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val KafkaParams = Map[String, String](
      ...
      "group.id" -> "rapidBusConsumerDecide",
  ...
  def fun(records: RDD[ConsumerRecord[String, String]]): Unit = {
    if (!records.isEmpty()) {
      ...
      records.foreachPartition { partitionRDD =>
        ...
        try{
          val jstat = jedis.hget("rapidStat","stat")
...
//            val agThreshold = avg_turn + ag_numThreahold * std_turn //转弯角度阈值
          ...
          partitionRDD.foreach(row => {
            ...
            //计算行驶结束
            val preStatus = jedis.hget("busEHstatus", jsy)
            if (preStatus == null) {
              jedis.hset("busEHstatus", jsy, curList(4) + ":" + curList(7))
            }
            else {
              ...
              if (curList(7) != preEH(1) && curList(4) > preEH.head) {
                ...
                if ((curList(7) == "2" && preEH(1) == "1") || (curList(7) == "0" && preEH(1) == "3")) {
                  ...
                  val cnt = Option(jedis.hget("busRapidDetail", jsy)) match {
                    case Some(s) => s
                    case _ => "0"
                  }
                  ...
                  jedis.hset("busRapidCnt", scoreKey, cnt)
                  statusProducer(scoreKey)
                  jedis.hdel("busRapidDetail", jsy)
                }
              }
            }
          })

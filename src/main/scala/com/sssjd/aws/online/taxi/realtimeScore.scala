      ...
      try {
        ...
        for (ary <- array) {
          ...
          if (topicMsg.contains(curList(0))) {
            ...
            if (status.equals(curList(1) + "_" + curList(2))) {
              /**
                * 读取redis获取超速、三急数据,进行评分，删除缓存数据
                */

              val overDetail = jedis.hget("OverSpeedInfo", ary).+(":0:2:3:4")
              val rapidDetail = jedis.hget("taxiRapidCnt", ary).+(":1:2:3")
              val overCnt = overSpeed(jedis, overDetail)
              val rapidCnt = rapidSpeed(jedis, rapidDetail)

              val timeInterval = curList(2).toLong - curList(1).toLong
              ...
              jedis.hdel("OverSpeedInfo", "uid_beginTime_endTime")
              jedis.hdel("taxiRapidCnt", "uid_beginTime_endTime")

            } else {
              println(topicMsg)
              topicMsg.update(curList(0), curList(1) + "_" + curList(2))
            }

          } else {
            println(topicMsg)
            println(curList(0))
            topicMsg.put(curList(0), curList(1) + "_" + curList(2))
          }
        }

...
object RapidSpeedJR extends EventFrequency {
//  val n_std_up = 2.6
//  val n_std_dn = 2.8
...
//  val n_std_angle = 0.8
  ...
  val angleLimit = 14.98
  val speedLimit = 28.95
  private val orient = 89.30
  private val month=7
  ...
  def main(args: Array[String]): Unit = {
    ...
    for(ary <- Array(s"JrRoadmonth$month") ){
      val dataSet: DataFrame = spark.read.option("encoding", "utf8").option("header", "true").csv(path+ary+"/0")

      val rapidDetail: DataFrame = calRapidSpeed(spark,dataSet)
      rapidDetail.repartition(1).write.mode(SaveMode.Overwrite).option("user",LoadConfig.getHdfsUser()).option("header", "true").csv(fsPath+s"JR_451_Data/JR/$month/Details/")

      val rapidDetailAndEvent = eventFrequency(spark,rapidDetail)
      rapidDetailAndEvent.repartition(1).write.mode(SaveMode.Overwrite).option("user",LoadConfig.getHdfsUser()).option("header", "true").csv(fsPath+s"JR_451_Data/JR/$month/statistics/")
  ...
  def calRapidSpeed(spark: SparkSession,dataSet: DataFrame): DataFrame = {
    ...
    val sql5 =
      s"""
         |select *,
         |case
         |when angleSpeed > $angleLimit and avgSpeed*3.6 > $speedLimit and orient>$orient then 3
         |when a_speed > $n_std_up and (actualDist > theroyDistUpMax or actualDist < theroyDistUpMin) and abs(actualDist - avgDist) > $threshold_gap_min and abs(actualDist - avgDist) < $threshold_gap_max and orient<30 then 1
         |when a_speed < $n_std_dn and (actualDist > theroyDistDnMax or actualDist < theroyDistDnMin) and abs(actualDist - avgDist) > $threshold_gap_min and abs(actualDist - avgDist) < $threshold_gap_max and orient<30 then 2
         |else 0
         |end as rapidMark
         |from tab_tmp2
         |having rapidMark <> 0

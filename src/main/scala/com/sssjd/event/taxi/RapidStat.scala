...
object RapidStat {
  ...
  case class Drive(jsy:String,lspeed:Double,lutc:String,longtime:Long ,llon:String,llat:String,orientation:String)
      ...
      ds.foreachRDD(rdd =>{
         if( !rdd.isEmpty()) {
            val df: DataFrame = rdd.map(_.split(",")).map(row => {
               Drive(row(1), row(2).toDouble, row(3), row(4).toLong, row(5), row(6), row(7))
              }).toDF()
            ...
            val sql =
              s"""
                 ...
                 |from ori_roadmatch
                 |where lspeed<=120
                 |having prelat <> 0 and prespeed <=120
              """.stripMargin
            spark.sql(sql).createOrReplaceTempView("ori_detail")

            // 计算加速度aspeed/方向角turn/角速度aturn/里程差distance/平均速度
            val sql_2 =
              s"""
                 |select
                 |tab.* ,
            ...
            //计算急加速加速度均值、标准差及四分位值；里程差均值、标准差及四分位值
            val sql_up =
            s"""
               |select
               |'st' as id,
            ...
            //计算急减速加速度均值、标准差及四分位值；里程差均值、标准差及四分位值
            val sql_dn =
            s"""
               |select
               |'st' as id,
            ...
            //计算急转弯角度差平均值；平均速度平均值、标准差；角速度平均值、标准差
            val sql_tn =
              s"""
                 |select
                 ...
                 |where turn > 45
                 """.stripMargin
            val avg_turn = spark.sql(sql_tn).first()(0).toString.toDouble
            ...
            s"""
               ...
               |where turn > $avg_turn
             """.stripMargin
            spark.sql(sql_tn2).createOrReplaceTempView("stat_tn")

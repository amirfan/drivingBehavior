...
object BusRapidDetail {

  private val acceThreshold = 0.503
  private val deceThreshold = -0.500
  private val mileageInterval_mi = 19.387
  private val mileageInterval_mx = 31.020
  private val speedLimit = 28.95 // --km/h
  private val angleLimit = 89.30
        ...
        try {
          partitionRDD.foreach(row => {
            ...
            if (preGPS.length == 10 && preGPS.head < preGPS(5) && preGPS(5) < curGps.head) {
              ...
              if (timeInterval >= 5 && timeInterval <= 60 && curGps(3).toFloat<80) {
                //急加急减条件条件：加速度大于阈值 && 里程差大于/小于临界值 && 里程大于/小于临界值
                //急转弯条件：角速度大于阈值 && 角度大于阈值 && 速度大于阈值
                var stat =
                if (sudSpeed >acceThreshold && (mileageInterval>mileageInterval_mi && mileageInterval<mileageInterval_mx) && ( actualMileage > theroyUpMax || actualMileage < theroyUpMin ) && angle<30)
                  curList ++ List("1")
                else if (sudSpeed < deceThreshold && (mileageInterval>mileageInterval_mi && mileageInterval<mileageInterval_mx) && ( actualMileage > theroyDnMax || actualMileage < theroyDnMin ) && angle<30)
                  curList ++ List("2")
